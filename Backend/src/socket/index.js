    ...
    export const initializeSocket = (httpServer) => {
        ...
        io.on("connection", (socket) => {
            ...
            // 🛠️ FIX 1: Grab the exact list of everyone sitting in the room right now
            const socketsInRoom = await io.in(roomId).fetchSockets()
            const activeUsers = socketsInRoom.map(s => s.user).filter(Boolean)

            // Send this absolute truth list to EVERYONE in the room (including the guy who just joined)
        ...
        // Code sync
            socket.on("code-change", ({ roomId, problemId, language, code }) => {
                ...
                roomCodeState.set(roomId, currentState);  
                // Emit language to everyone else
            ...
            if (!roomId || !problemId) {
             ...
             try {
                ...
                // 🛠️ FIX: Save the active problem to memory so refresh/late-joiners get the right one
                const currentState = roomCodeState.get(roomId) || { codes: {}, languages: {} }
